#!/bin/bash
# =============================================================================
# Entrypoint inventory-database — initialisation idempotente.
#
...
if [ -z "$(ls -A ${PG_DATA} 2>/dev/null)" ]; then
    echo ">>> [inventory-database] Premier démarrage — initialisation de la base"
...
EOSQL
    then
        echo ">>> [inventory-database] ERREUR : l'initialisation SQL a échoué" >&2
        /usr/lib/postgresql/${PG_VERSION}/bin/pg_ctl -D ${PG_DATA} -m fast -w stop || true
        exit 1
    fi

    echo ">>> [inventory-database] Initialisation réussie"
    /usr/lib/postgresql/${PG_VERSION}/bin/pg_ctl -D ${PG_DATA} -m fast -w stop
else
    echo ">>> [inventory-database] Volume déjà initialisé, démarrage direct"
fi
